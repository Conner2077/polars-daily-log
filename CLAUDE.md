# Auto Daily Log — 项目核心原则

> 本文面向参与本项目的 AI 助手与开发者。所有设计决策、prompt 改动、UI 流程改动都应遵守这些原则。
...
- Server 核心必须跨平台。平台专属的监控逻辑放在 `auto_daily_log_collector/platforms/<os>.py`
- Collector 与 Server 通过 HTTP 协议通信，任意平台都能做 Collector
...
- 反侦测 app（如企业微信）要在 `platforms/<os>.py` 的 `_HOSTILE_APPS` 白名单里避开深度 introspection
